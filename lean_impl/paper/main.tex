...
cuts alone --- no branching --- which is why the checked certificates use only
\code{lin} and \code{rnd}. This exercises the full chain on genuine, untrusted solver
certificates.
...
example's leaves --- and the case-split rule \code{uns} is proved sound and available;
wiring \code{uns} into the sensed replay (so branched certificates are also accepted end
to end) is in progress.
...
the number of neurons the leaf leaves unfixed --- the leaf-aware encoding at work. Two
certificate families appear: signed combinations (\code{lin}) and Gomory rounding cuts
(\code{rnd}); on this network \scip{} needs no branching. Every certificate is genuine,
untrusted solver output, re-checked in exact~$\Q$ by the verified checker.
...
re-checked by the kernel-checked \code{leafCheck}).}
\label{tab:eval}
\begin{tabular}{lrrrrl}
\toprule
Leaf & \#vars & \#int & \#constraints & \#\vipr{} steps & reasons used \\
\midrule
...
\midrule
\multicolumn{6}{l}{\emph{Verdict:} coverage OK; all leaves \sound{}
