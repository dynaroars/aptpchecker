...
\item \textbf{Affine equalities:} each layer's output equals the linear function of its
      input, e.g.\ $v_2=-0.5v_0+0.2v_1-0.5$.
\item \textbf{ReLU encoding for the unbranched $N_3$:} four inequalities using a $0/1$
      helper variable $v_7$ to say ``$v_6=\max(v_4,0)$''; these use the range from
      Step~4 ($[-0.43,0.86]$) as the ``big-$M$'' constants.
\item \textbf{Sign constraints:} the leaf says $N_1,N_2,N_4\ge0$, so $v_2\ge0$,
      $v_3\ge0$, $v_5\ge0$.
\item \textbf{Negated property:} the objective $v_8-v_9\le\rho$ (the opposite of what
      we want to prove).
\end{itemize}
In total: 23 rows + the objective row, over 15 variables.

\emph{What we proved (\code{encoding\_overapprox\_mlp}).} If you plug in the
...
appears verbatim as \code{V0 R4 1/2}), declares \emph{only} the ReLU indicator as
integer, and invokes exact \scip{}, which returns a \vipr{} certificate. Our verified
\code{checkSem} then re-checks each certificate. The console output:
\begin{center}\small
\begin{tabular}{l}
\texttt{Coverage: OK (leaves tile the activation-pattern cube)}\\
\texttt{Objective 0: c=[1, -1], rhs=0}\\
\texttt{~~leaf 0: CERTIFIED (verified checkSem accepts SCIP's certificate)}\\
\texttt{~~leaf 1: CERTIFIED \dots\quad leaf 2: CERTIFIED\quad leaf 3: CERTIFIED}\\
\texttt{CERTIFIED: $\forall$ x $\in$ dom, c $\cdot$ net(x) > $\rho$}\\
\end{tabular}
\end{center}
The four certificates \scip{} produced are real solver output, not hand-written: they
range from $97$ to $802$ derivation steps over $22$--$38$ constraints and $9$--$15$
variables, and \emph{leaf 1 uses two Gomory rounding cuts} (\code{rnd} steps) alongside
the signed-multiplier combinations (\code{lin}); \scip{} proved every leaf without
branching. All four are accepted by the kernel-checked \code{checkSem}
(\S\ref{sec:certificates}), whose soundness theorem (\code{checkSem\_sound}) is
axiom-clean. This exercises the full chain on genuine, untrusted solver certificates.
...
each leaf as an exact-rational \code{.mps}, exact \scip{} returns a \vipr{} certificate,
and the kernel-checked \code{checkSem} validates it. All four leaves are accepted and
the tool reports \sound{} (Step~10, \S\ref{sec:example}). Table~\ref{tab:eval} gives the
real per-leaf certificate sizes: they span $97$--$802$ derivation steps; three leaves are
pure signed-combination proofs (\code{lin}) and one additionally uses two Gomory rounding
cuts (\code{rnd}); none required branching. Every certificate is genuine, untrusted
solver output, re-checked in exact~$\Q$ by the verified checker.

\begin{table}[t]
\centering
\caption{Real end-to-end run of \tool{} on the worked example with official exact
\scip{} 10.0.1. Each certificate is re-checked by the kernel-checked \code{checkSem}.}
\label{tab:eval}
\begin{tabular}{lrrrl}
\toprule
Leaf & \#vars & \#constraints & \#\vipr{} steps & reasons used \\
\midrule
$\leaf_1=[-4]$      & 15 & 38 & 97  & \code{lin} \\
$\leaf_2=[-2,4]$    & 13 & 33 & 174 & \code{lin}, \code{rnd}$\times2$ \\
$\leaf_3=[2,1,4]$   & 10 & 26 & 123 & \code{lin} \\
$\leaf_4=[2,-1,4]$  & 9  & 22 & 802 & \code{lin} \\
\midrule
\multicolumn{5}{l}{\emph{Verdict:} coverage OK; all leaves \sound{} $\Rightarrow$
$\forall x\in\dom.\ Y_0>Y_1$.} \\
\bottomrule
\end{tabular}
\end{table}
