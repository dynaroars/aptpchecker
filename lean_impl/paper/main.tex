...
central theorem (mechanized for one-hidden-layer networks; general depth in progress,
see \S\ref{sec:formalization} ``Mechanization status''):
...
\emph{Encoding soundness} (Lemma~\ref{lem:enc}) is proved for the affine case
(\code{certified\_sound\_singleLinear}, a single fused linear layer) and for the
\textbf{one-hidden-layer} class $\mathrm{Lin}\!\to\!\relu\!\to\!\mathrm{Lin}$ with the
\emph{full} big-$M$ encoding: \code{encoding\_overapprox\_oneHidden} builds the real
trace and shows it satisfies every emitted row (box, both affine layers, the four
big-$M$ rows per neuron, and the leaf's sign rows) with the objective evaluating to
$c\cdot\net(x)$, and \code{certified\_sound\_oneHidden} composes it with
\code{refute\_of\_cert} to give per-leaf refutation---all axiom-clean.
...
Remaining, and none requiring trust in \scip{} or floats: the general \code{List
Layer} depth induction (the one-hidden-layer proof supplies the per-layer building
blocks); threading \code{rnd}/\code{uns} into the \emph{automatic} fold (they are
proved as standalone lemmas) with assumption-set bookkeeping; and the executable CLI
wiring into Fig.~\ref{fig:thm}.
