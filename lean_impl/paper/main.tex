...
runnable \code{checkRefTree}); and the composition scaffolding plus the VIPR endpoint
(\code{Pipeline/Compose}, \code{Pipeline/Affine}, \code{Pipeline/ViprCheck}). Under
construction: bridging the dimension-indexed \code{MLP} soundness model to the
executable \code{Array} encoder, Conv/CNN, and the command-line driver
...
check, and---out of the trusted base---both input parsers by round-trip. What remains
is engineering, not new trust: bridging the dimension-indexed soundness model to the
executable encoder, assembling the command-line driver (with an untrusted
