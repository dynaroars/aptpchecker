...
\emph{Certificate replay} is mechanized at the step level: \code{lin\_sound} (a
nonnegative combination of $\le$-rows is implied, generalizing
Lemma~\ref{lem:farkas}), \code{rnd\_sound} (integer rounding of a bound under
integrality), and \code{uns\_sound} (discharge of an integer split
$x_i\le k \lor x_i\ge k{+}1$) are all proved axiom-clean, alongside a VIPR~v1.0 parser
and a checked \code{lin}-only replay fold (\code{replay\_infeasible\_all}).
...
sample). The `.net` parser is thus out of the trusted base.

Remaining, and none requiring trust in \scip{} or floats: threading
\code{rnd}/\code{uns} into the \emph{automatic} replay fold (they are proved as
standalone lemmas) with assumption-set bookkeeping; bridging the dimension-indexed
\code{MLP} soundness model to the executable \code{Array}-based encoder; verifying the
\code{.aptp} parser (as done for \code{.net}); Conv/CNN encoding; and the executable
CLI wiring into Fig.~\ref{fig:thm}.

\paragraph{Trusted computing base.} The TCB is: the \lean{} kernel and $\Q$
arithmetic; the \code{.aptp} parser (differential-fuzzed against the reference Python
parser; verification is future work)---the \code{.net} parser is now \emph{fully
verified} (\code{parseNet\_printNet}) and out of the TCB; and the \code{.net} file
faithfully representing the intended network (its generation is out of scope---the
tool reads only the exact format). \scip, SoPlex, the \vipr{} certificate, and any
certificate-completion tooling are \emph{not} trusted.
