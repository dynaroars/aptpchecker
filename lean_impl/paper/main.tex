...
original property holds for \emph{every} input in the box. We report the current
mechanization: the Farkas certificate check, the coverage check, and the two
encoding-soundness cornerstones are machine-checked and axiom-clean, and the exact
evaluator is validated bit-for-bit against an independent oracle. We describe the
formalization, the trusted computing base, and the integration with \scip/\vipr.
...
\begin{itemize}
  ...
  \item We remove the trusted solver. We run the \emph{exact} solver \scip{}
    ...
    rationals; SCIP leaves the trusted computing base entirely
  ...
  \item We isolate \emph{encoding soundness}---that the rational constraint system
    ...
    theorem, and mechanize its two cornerstones (soundness of the big-$M$ ReLU
    relaxation and validity of the exact interval bounds) for feed-forward \relu{}
    networks (\S\ref{sec:formalization}).
...
We instantiate every step on the toy network \code{sample.onnx} and proof
\code{sample.aptp} shipped with the reference tool. All arithmetic is exact;
decimals such as $0.2$ are shown for readability but the checker carries the exact
binary32 value $\mathtt{0x3E4CCCCD}=13421773/67108864$. The exact-$\Q$ network
outputs below are validated bit-for-bit against an independent rational forward
pass.
...
\paragraph{Neuron stability.} Propagating $\dom$ with the exact interval rule
...
\]
\emph{All four neurons are unstable.} These loose bounds supply the big-$M$
constants; any tighter sound bound only helps.
...
\paragraph{Coverage.} At any $x$ each pre-activation is $\ge 0$ or $<0$, so
$(N_1,N_2,N_4)\in\{0,1\}^3$ is \emph{some} assignment; we need the DNF to be a
tautology over that cube. Case split: $N_4{=}0\Rightarrow\leaf_1$; else $N_4{=}1$ and
$N_2{=}0\Rightarrow\leaf_2$; else $N_2{=}1$ with $N_1{=}1\Rightarrow\leaf_3$,
$N_1{=}0\Rightarrow\leaf_4$. All eight assignments are covered, so the leaves tile
$\dom$ (Lemma~\ref{lem:cover}). $N_3$ is irrelevant: both its signs lie inside each
region.

\paragraph{Per-leaf refutation.} For each leaf we prove the system
$\{x\in\dom\}\cup\{\text{affine equalities}\}\cup\{\text{big-}M\ \relu\}\cup
\{\text{leaf sign fixings}\}\cup\{c\cdot y\le 0\}$ infeasible. Because $N_3$ keeps its
binary, this is a MILP; exact \scip{} emits a \vipr{} tree of \code{lin}/\code{rnd}/%
\code{uns} steps that we replay in $\Q$, bottoming out in Lemma~\ref{lem:farkas}. To
see a certificate concretely, fix the \emph{complete} pattern
$N_1,N_2,N_3,N_4\ge 0$: every \relu{} is the identity and $\net$ is affine, with
\[
y_0-y_1 \;=\; 0.2\,h^{(2)}_0-0.3\,h^{(2)}_1+0.1
\;=\; 0.2\,p^{(2)}_0-0.3\,p^{(2)}_1+0.1,
\]
an explicit affine $\alpha x_0+\beta x_1+\gamma$ after substituting
$p^{(2)}=W_2h^{(1)}+b_2$, $h^{(1)}=p^{(1)}=W_0x+b_0$; refuting $y_0-y_1\le 0$ is then a
single \code{lin} step (a nonnegative combination of the box and sign bounds yielding
$0\le-\varepsilon$). This LP-only, complete-pattern mode is the earliest milestone;
the actual partial leaves add one integer split (\code{uns}) for $N_3$.
...
\scip{} could certify a different problem. This byte-level correspondence is part of
$\tool$'s validation and is what links Lemma~\ref{lem:farkas}'s conclusion back to
$\reg(\leaf)$.
...
\paragraph{Mechanization status.} The following are proved and \emph{axiom-clean}
(depending only on \code{propext}, \code{Classical.choice}, \code{Quot.sound}---no
\code{sorry}, no \code{native\_decide}): the Farkas checking direction
(Lemma~\ref{lem:farkas}, \code{farkas\_infeasible}), the coverage check
(Lemma~\ref{lem:cover}, \code{coverage\_sound}, with a complete Boolean-cube
enumeration proof), and the two encoding cornerstones (Lemmas~\ref{lem:relu}
and~\ref{lem:ivl}). The exact substrate is validated operationally: the $\Q$
...
Remaining, and none requiring trust in \scip{} or floats: bridging the
dimension-indexed \code{MLP} soundness model to the executable \code{Array}-based
encoder; Conv/CNN encoding; and the executable CLI wiring into Fig.~\ref{fig:thm}
(including the untrusted flat-VIPR-to-tree converter).
...
\tool{} is a \lean{}~4 package (currently $\approx$1.5k lines, growing) built on
\mathlib. Implemented: the exact numeric layer (\code{Numeric/Float}), the
S-expression lexer and the \code{.aptp}/\code{.net}/\vipr{} parsers (\code{Ast/*}), the
exact network evaluator (\code{Model/Network}), the encoding cornerstones and the
executable encoder (\code{Model/Encoding}, \code{Model/Encoder}), the one-hidden-layer
encoding-soundness proof (\code{Model/EncodingSound}), the Farkas check
(\code{Cert/LinComb}), the coverage check (\code{Coverage/Tautology}), and the
\code{lin}/\code{rnd}/\code{uns} certificate-step soundness with a \code{lin}-replay
fold (\code{Cert/Vipr}), and the stage-by-stage `.net`-parser round-trips
(\code{Ast/NetRoundtrip}). Under construction: the general-depth encoding induction, the
automatic threading of \code{rnd}/\code{uns} into the replay fold, and the executable
driver composing the pieces into Theorem~\ref{thm:main} (\code{Pipeline/*}). A
command-line driver
\code{aptpcheck -{}-net N.net -{}-aptp P.aptp -{}-vipr-dir D/} parses the inputs,
validates each leaf certificate and the coverage condition, and prints the verdict; on
\sound{} the accompanying \lean{} theorem is kernel-checkable. The proof format and
neuron numbering match \pytool, so its proofs are consumed unchanged (given exact
\vipr{} certificates from \scip).
...
kernel-checked, exact, end-to-end soundness proof for a \emph{verifier-independent}
...
into per-leaf refutation, coverage, and encoding soundness; the Farkas check, the
coverage check, and the two encoding cornerstones are already machine-checked and
axiom-clean. Remaining work completes the end-to-end theorem---the encoding assembly
and the \vipr{} \code{lin}/\code{rnd}/\code{uns} replay and their composition---after
which we target \emph{completeness} (via \mathlib's Farkas/LP duality), a verified
parser, convolutional and residual architectures at scale, and parallel certificate
checking.
