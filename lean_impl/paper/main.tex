...
\paragraph{Untrusted \scip{}, checked \vipr{}.} We hand the system to exact \scip{},
which returns a \vipr{} certificate of infeasibility. Re-checking that certificate is a
self-contained problem, independent of neural networks, and we treat it in a companion
paper~\cite{companion}: a kernel-checked \lean{} checker, \code{checkSem}, replays the
\code{DER} list over $\Q$---signed \emph{suitable} combinations, \code{rnd} rounding
under integrality, and \code{uns} branch-and-bound case-splits with assumption
discharge---and its soundness theorem \code{checkSem\_sound} (axiom-clean) states that
acceptance implies the certified system is infeasible over the integers. Here we use it
as a black box with that guarantee. Its only non-arithmetic ingredient is the soundness
(\emph{easy}) direction of Farkas, which is also the two-line core of the per-leaf
argument:
...
\emph{Certificate replay} is a complete, automatic, runnable, kernel-checked checker
\code{checkSem}, whose full development---the VIPR~v1.0 calculus (signed suitable
combinations, domination, Gomory rounding, and branch-and-bound case-splits with
assumption discharge), the flat replay, and the soundness theorem
\code{checkSem\_sound}---is the subject of the companion paper~\cite{companion}. We
summarize only what this paper consumes: \code{checkSem} replays the certificate's own
\code{DER} list over $\Q$ and \code{checkSem\_sound} (axiom-clean) states that
acceptance implies the certified constraint system is infeasible over the integers. It
is validated on \emph{real} certificates from
